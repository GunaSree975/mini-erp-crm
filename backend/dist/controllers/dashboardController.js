...
const getDashboardStats = async (req, res) => {
    try {
        const [totalCustomers, activeCustomers, leadsCount, totalProducts, allProducts, totalChallans, confirmedChallans, draftChallans, stockMovementsCount, recentChallans, recentMovements,] = await Promise.all([
        ...
        const totalInventoryValuation = allProducts.reduce((sum, p) => sum + p.currentStock * p.unitPrice, 0);
